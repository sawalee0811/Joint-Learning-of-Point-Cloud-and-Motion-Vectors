...
def densify_default(params, variables, optimizer, i):
    return params, variables

def compute_isolation_loss_cpu_auto(points, k=3, margin_ratio=2.0):
    """
    Auto version:
    For each point, get mean kNN distance.
    If mean distance > global_mean * margin_ratio, penalize.
    """
    points_cpu = points.detach().cpu().numpy()
    tree = cKDTree(points_cpu)
    dists, _ = tree.query(points_cpu, k=k+1)  # k+1 to include self
    mean_dists = dists[:, 1:].mean(axis=1)    # skip self

    global_mean = mean_dists.mean()
    excess = np.clip(mean_dists - (global_mean * margin_ratio), a_min=0.0, a_max=None)
    isolation_loss = torch.tensor(excess.mean(), device=points.device)
