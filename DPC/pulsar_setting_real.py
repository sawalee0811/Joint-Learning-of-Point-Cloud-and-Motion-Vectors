...
# ---- Utilities (APIs unchanged) ----
from helpers import o3d_knn

BATCH_SIZE = 6
VELOCITY_INIT = False  # True => pts += (pts - prev_pts)

# === Global scale to match render.py (WORLD_SCALE = 1 / IMAGE_SIZE = 1/600) ===
WORLD_SCALE = 1.0 / 500.0

def load_npz(file_path, device, r):
    """
    Load point cloud from .npz/.npy. Returns (xyz, rgb, radius).
    - xyz is scaled by WORLD_SCALE (1/600) to match cam.json scale.
    - rgb is normalized to [0,1] if input is 0-255.
    - radius is also scaled for consistency with xyz in this module.
      (Note: train.py will create the rendering radius separately and
       also apply WORLD_SCALE there; this radius here is used for seg_colors.)
    """
